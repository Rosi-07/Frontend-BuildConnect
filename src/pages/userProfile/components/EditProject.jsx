...
const EditProject = ({ isOpen, onClose, project }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.put(`projects/${project.id}`, { ...projectData, UserId: auth?.user?.id });
            ...
            enqueueSnackbar(
                `Error al actualizar proyecto: ${error.response?.data?.message || "Error desconocido"}`,
                { variant: "error",
                  anchorOrigin: {
                    vertical: "top",
                    horizontal: "right",
